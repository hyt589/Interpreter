...
(define M_state_funcall
  (lambda (funcallstat state return whileReturn throwReturn breakReturn)
    (cond
      ((null? (cdr state)) (run (getFourth (lookupfunc (getSecond funcallstat) state))
                                (createFuncLayer (getThird (lookupfunc (getSecond funcallstat) state)) (getAfterSecond funcallstat) (addlayer '() state)) return whileReturn throwReturn breakReturn))
      ...
      (else (run (getFourth (lookupfunc (getSecond funcallstat) state))
                 (cons (getFirst (createFuncLayer (getThird (lookupfunc (getSecond funcallstat) state)) (getAfterSecond funcallstat) (addlayer '() state))) state) return whileReturn throwReturn breakReturn)))))
...
; this function creates a proper new layer for the function call
...
;this function checks whether the input list matches the param list of the function called
...
;bind each parameter with its respective input value
